...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_mut_instrument)) {
    StringRef val = A->getValue();
    if (val == "winmut")
      CmdArgs.push_back("-winmut-cc1");
    else if (val == "accmut")
      CmdArgs.push_back("-accmut-cc1");
    else if (val == "runtime")
      CmdArgs.push_back("-winmut-runtime-cc1");
    else if (val == "time")
      CmdArgs.push_back("-winmut-time-cc1");
  }

  if (const Arg *A = Args.getLastArg(options::OPT_winmut_optimized_instrument)) {
    StringRef val = A->getValue();
    if (val == "false")
      CmdArgs.push_back("-winmut-no-opt-cc1");
  }

  if (const Arg *A = Args.getLastArg(options::OPT_mut_maxBBSize_EQ)) {
    StringRef val = A->getValue();
    CmdArgs.push_back(Args.MakeArgString("-mut-maxBBSize=" + val));
  }
