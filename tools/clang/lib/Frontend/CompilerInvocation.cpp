...
static bool ParseCodeGenArgs(CodeGenOptions &Opts, ArgList &Args, InputKind IK,
  ...
  if (Args.hasArg(OPT_winmut_cc1)) {
    Opts.setWinMutInstrument(CodeGenOptions::WinMut);
  }
  if (Args.hasArg(OPT_accmut_cc1)) {
    Opts.setWinMutInstrument(CodeGenOptions::AccMut);
  }
  if (Args.hasArg(OPT_winmut_raw_runtime_cc1)) {
    Opts.setWinMutInstrument(CodeGenOptions::WinMutRuntime);
  }
  if (Args.hasArg(OPT_winmut_time_cc1)) {
    Opts.setWinMutInstrument(CodeGenOptions::WinMutMeasureTime);
  }
  if (Args.hasArg(OPT_winmut_no_opt_instrument_cc1)) {
    Opts.WinMutOptimization = 0;
  }
  Opts.maxBBSize = getLastArgIntValue(Args, options::OPT_mut_maxBBSize_EQ_cc1, 15);
