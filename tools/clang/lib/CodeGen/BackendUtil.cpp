...
#include "llvm/Transforms/WinMut/Config.h"
// #include "llvm/Transforms/WinMut/DMAInstrumenter.h"
#include "llvm/Transforms/WinMut/InstrumentMainPass.h"
// #include "llvm/Transforms/WinMut/MSInstrumenter.h"
...
// #include "llvm/Transforms/WinMut/StatisticsUtils.h"
...
void EmitAssemblyHelper::EmitAssembly(BackendAction Action,
  ...
  {
    PrettyStackTraceString CrashInfo("WinMut work passes");
    if (CodeGenOpts.getWinMutInstrument() ==
        CodeGenOptions::WinMutMeasureTime) {
      PerModulePasses.add(new AddMainArgcArgvPass());
      PerModulePasses.add(new InstrumentMainPass("__accmut__init_timing"));
    } else if (CodeGenOpts.getWinMutInstrument() !=
               CodeGenOptions::NoWinMutInstrument) {
      PerModulePasses.add(new PrintPass(".ll"));
      
      PerModulePasses.add(new NaiveSplitBlockPass(CodeGenOpts.maxBBSize));
      PerModulePasses.add(new PrintPass(".split.ll"));
      ...
      switch (CodeGenOpts.getWinMutInstrument()) {
      case CodeGenOptions::AccMut:
        PerModulePasses.add(new WAInstrumenter(false, CodeGenOpts.WinMutOptimization));
        break;
      case CodeGenOptions::WinMut:
        PerModulePasses.add(new WAInstrumenter(true, CodeGenOpts.WinMutOptimization));
        break;
      default:
        break;
      }
      ...
      PerModulePasses.add(new InstrumentMainPass("__accmut__init"));
...
#ifdef HAVE_POST_OPTIM

  if (CodeGenOpts.getWinMutInstrument() != CodeGenOptions::NoWinMutInstrument) {
    legacy::PassManager PerModulePassesPure;
    PerModulePassesPure.add(
        createTargetTransformInfoWrapperPass(getTargetIRAnalysis()));
    legacy::FunctionPassManager PerFunctionPassesPure(TheModule);
    PerFunctionPassesPure.add(
        createTargetTransformInfoWrapperPass(getTargetIRAnalysis()));

    CreatePasses(PerModulePassesPure, PerFunctionPassesPure);
    PerModulePassesPure.add(new PrintPass(".post.ll"));
    {
      PrettyStackTraceString CrashInfo("Per-function post optimization");

      PerFunctionPassesPure.doInitialization();
      for (Function &F : *TheModule)
        if (!F.isDeclaration())
          PerFunctionPassesPure.run(F);
      PerFunctionPassesPure.doFinalization();
    }

    {
      PrettyStackTraceString CrashInfo("Per-module post optimization passes");
      PerModulePassesPure.run(*TheModule);
    }
  }
#endif
